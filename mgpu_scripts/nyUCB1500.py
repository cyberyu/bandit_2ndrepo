...
def train(epochs, log=False):
    ...
    X_nystrom_text = torch.tensor(K_pc_500_text, dtype=torch.float32).to(cuda1)
    X_nystrom_title = torch.tensor(K_pc_500_title, dtype=torch.float32).to(cuda2)
    
    # parameters
    A_text = torch.ones([num_articles, 1, 1]).to(cuda1) * torch.eye(num_arm_features).to(cuda1)
    A_title = torch.ones([num_articles, 1, 1]).to(cuda2) * torch.eye(num_arm_features).to(cuda2)
    
    b_text = torch.ones([num_articles, 1, 1]).to(cuda1) * torch.zeros([num_arm_features, 1]).to(cuda1)
    b_title = torch.ones([num_articles, 1, 1]).to(cuda2) * torch.zeros([num_arm_features, 1]).to(cuda2)
...
#             print(query_id)
            ...
            p_t_all = torch.stack([p_t_a_text, p_t_a_title], 2).reshape(-1,1,2)
            ...
            del p_t_a_text, p_t_a_title, v_t, h_t
            torch.cuda.empty_cache()
...
#             print(query_test_id)
            ...
            with torch.cuda.stream(s2):
                ...
                torch.cuda.empty_cache()
            torch.cuda.synchronize() 
            predict_score = torch.max(torch.stack([predict_score_text, predict_score_title], dim=1), dim=1).values
            
            del predict_score_text, predict_score_title
            torch.cuda.empty_cache()
