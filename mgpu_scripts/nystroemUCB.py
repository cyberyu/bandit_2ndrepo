...
def body1(A, b, epoch_id, rewards):
    global X_page, X_query, X_ratings, X_nystrom, query_features, page_features, test_query_features, test_ratings
    ...
    def get_arm_features(qt_id):
        ...
        return tf.expand_dims(tf.gather_nd(X_nystrom, tf.convert_to_tensor(qt_id_to_arm_ids)), -1)
    ...
    def body2(q_id, A, b, rewards, epoch_id):
        ...
        def get_arm_features_v2(qt_id, a_t):
            ...
            return tf.expand_dims(tf.gather_nd(X_nystrom, tf.convert_to_tensor([[a_t[0][0].numpy()*num_articles+qt_id.numpy()]])), -1)
...
#                     import IPython; IPython.embed(); exit(1)
                    ...
                    #print(binomial_reward_probability.numpy())
...
#         x_t = tf.expand_dims(arm_f, -1) # add a new dimension
        ...
        a_t_series=tf.cast(tf.nn.top_k(tf.transpose(p_t_a), k=3).indices[0][0],tf.int64)
        
        a_t = tf.expand_dims(tf.expand_dims(a_t_series[0],-1),-1)
        ...
        A_a_t_new = tf.add(tf.cast(A_a_t, float), tf.cast(tf.matmul(tf.reshape(x_t_at_nystroem,[num_arm_features,-1]), tf.transpose(tf.reshape(x_t_at_nystroem,[num_arm_features,-1]))), float))

        b_a_t = tf.gather_nd(b, a_t)
        b_a_t_new = tf.add(b_a_t,r_t*tf.cast(x_t_at_nystroem,float))
        
        A=tf.tensor_scatter_nd_update(A, a_t, A_a_t_new)
        b=tf.tensor_scatter_nd_update(b, a_t, b_a_t_new)
        
#         a_t = tf.expand_dims(tf.expand_dims(a_t_series[1],-1),-1)
#         r_t = recommend(query_id=q_id, page_id=a_t, ratings=ratings)
...
#         a_t = tf.expand_dims(tf.expand_dims(a_t_series[2],-1),-1)
#         r_t = recommend(query_id=q_id, page_id=a_t, ratings=ratings)
...
#         b=tf.tensor_scatter_nd_update(b, a_t, b_a_t_new)
    ...
    repeat = 341
    test_score = tf.Variable(tf.zeros([num_articles,repeat]))    
    ...
    query_loop_condition_test = lambda query_test_id, A, b, test_score: tf.less(query_test_id, tf.constant(repeat))
    ...
    print(f'Epoch {epoch_id} | Train Rewards {rewards.numpy()[0]} | AUC {auc_score}')
    print(f'Time {t1 - t0}')
    
    with tf.device('/cpu'):
        try:
            tf.summary.scalar('Train Rewards', data=rewards.numpy()[0], step=tf.cast(epoch_id, tf.int64))
...
def train(epochs, oldA=None, oldb=None):
...
#     query_features=intent_features
    ...
    elif ((oldA is not None) and (oldb==None)):
        A=tf.Variable(np.load(oldA), name='A')
        bv = tf.Variable(tf.zeros([num_arm_features, 1], name='bv'))
        b = tf.Variable(tf.ones([num_articles, 1, 1], name='b') * bv)
    elif ((oldA==None) and (oldb is not None)):
        A = tf.Variable(tf.ones([num_articles, 1, 1], name='A') * ident)
        b=tf.Variable(np.load(oldb),name='b')
    else:
        A=tf.Variable(np.load(oldA), name='A')
        b=tf.Variable(np.load(oldb), name='b')
...
#eid, re, A, b = train(400, 'trainrecord_A.npy','trainrecord_b.npy')
eid, re, A, b = train(1000)
# Epoch 1 | Train Rewards [355.] | AUC 0.581616967316478
# Time 1246.682743549347


np.save('trainrecord_A.npy', A.numpy(), allow_pickle = False)
np.save('trainrecord_b.npy', b.numpy(), allow_pickle = False)
