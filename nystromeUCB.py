        ...
        def recommend(query_id, page_id, ratings):
            ...
            def if_false(): 
                ...
                def return_normalprob():
                    ...
                    # 
        #             pos_indices = [[x, y[0].numpy()] for x in range(X_ratings.shape[0]) for y in query_pos_rat_idxs]
        #             print('pos_indices', pos_indices)
        #             print(X_query.shape)
        #             print('before tp1')
        #             tp1 = tf.gather_nd(X_query,pos_indices)
        #             print('after tp1')
                    ...
                    #tp2 = tf.reshape(tf.nn.l2_normalize(current_query_features, 0),[25,1])
                    tp2 = tf.reshape(current_query_features,[query_feature_num,1])
        #             print('after tp2')
        #             print(tp1)
                    #tp3 = tf.reshape(tf.nn.l2_normalize(tp1, 0),[25,1])
        #            print('before match likable')
        #             print(tp2)
        #             print(tp3)
...
#         tf.compat.v1.scatter_update(b,[a_t],b_a_t_new)        
    ...
    test_score = tf.Variable(tf.zeros([321,986]))    
    repeat = 986
    ...
    query_loop_condition_test = lambda query_test_id, A, b, test_score: tf.less(query_test_id, tf.constant(986))
...
#     query_features=intent_features
    ...
    query_features = pickle.load(open('data/X_query_tfidf.pkl','rb'))
    page_features=pickle.load(open('data/X_page_title_tfidf.pkl','rb'))
    
    test_query_features= pickle.load(open('data/X_query_tfidf.pkl','rb'))

    ratings = pickle.load(open('data/X_ass_mat.pkl','rb'))
    test_ratings = pickle.load(open('data/X_ass_mat.pkl','rb'))
    
    K_pc_2000 = pickle.load(open('data/K_pc_2000_text.pkl','rb'))
...
#         ratings[i,j] = 1
    ...
    X_nystrom = tf.constant(K_pc_2000, dtype=tf.float32, name="X_nystrom")
